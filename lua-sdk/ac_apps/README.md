...
Attempts to open race menu (only available if car is in pits).
  ...
  1. `tab`: `nil|'race'|'setup'|'telemetry'|'time'|nil` Default value: `nil`.
...
currently supported.
  ...
  1. `config`: `'controls.ini'|'video.ini'|'assetto_corsa.ini'|'graphics.ini'|'graphics_adjustments.ini'|'custom_rendering_modes.ini'`
...
## Function ac.getRealMirrorAABB(mirrorIndex, min, max)
  ...
  - `boolean`
## Function ac.setRealMirrorParams(mirrorIndex, paramsPtr, userDriven)
...
it in combination with `ac.getRealMirrorParams()`:
 ...
 If called before with `userDriven` set to `false`, any subsequent calls with `userDriven` set to `true` will be ignored.
  ...
  2. `paramsPtr`: `ac.RealMirrorParams` New params to apply.
...
## Function ac.exportLapTimesToCSV()
...
manually (just add a new line with it in “.vscode/settings.json”).
  ...
  2. `data`: `serializable|nil` Data to pass to the script (will be available as `__input` value). Use `stringify()` if you need to pass something complex. Default value: `nil`.
